...
ggplot(data.frame(x = c(0.01, 0.2)), aes(x = x)) +
  stat_function(fun = function(lambda) logLn(lambda), size = 1) +
  geom_point(aes(x=1/mean(t), y=logLn(1/mean(t))), colour=palette.colors(2), size = 3) +
  theme_minimal() +
  xlab("lambda") + 
  ylab("log likelihood")
```
...
ggplot(data.frame(x = c(0, 4)), aes(x = x)) +
  ...
  geom_point(aes(x=betahat, y=logLOurSample(betahat)), colour = palette.colors(2), size = 3) +
  theme_minimal() +
  xlab("beta") +
  ylab("log likelihood")
```
...
ggplot(data.df, aes(x = x, y = y)) +
  geom_point() +
  stat_function(fun = f, colour = palette.colors(2)[2], size = 1) +
  theme_minimal()
```
...
ggplot(data.frame(wald), aes(x = wald)) +
  ...
  stat_function(fun = dnorm, size = 1.0, color = palette.colors(2)[2]) +
  theme_minimal() + 
  xlab("wald statistic")
```
