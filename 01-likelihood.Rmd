...
L(p) := p^3(1-p)^7.
$$
```{r likelihood, cache = FALSE, echo = FALSE, fig.cap='Likelihood of the sample', out.width='80%', fig.asp=.75, fig.align='center'}
library(ggplot2)

ggplot(data.frame(x = c(0.0, 1.0)), aes(x = x)) +
stat_function(fun = function(p) p^3*(1-p)^7, size = 1) +
  theme_minimal() +
  xlab("p") + 
  ylab("likelihood")
...
ggplot(data.frame(x = c(0.01, 0.2)), aes(x = x)) +
  stat_function(fun = function(lambda) logLn(lambda, t), size = 1) +
...
ggplot(data.frame(x = c(0, 4)), aes(x = x)) +
  stat_function(fun = Vectorize(logLn), 
                size = 1) +
