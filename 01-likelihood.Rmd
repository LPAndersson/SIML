...
```{r exp-obs, echo = FALSE, tidy=FALSE, cache=FALSE}
set.seed(1.0)
numObs = 100
actualRate= 0.12
...
```{r exp-boxplot, echo = FALSE, tidy=TRUE, cache=FALSE, message=FALSE, fig.cap = "Histogram of the sample", out.width='80%', fig.asp=.75, fig.align='center'}
library(ggplot2)
ggplot(t.df, aes(x = t)) +
  geom_histogram() +
  theme_minimal()
...
```{r log-likelihood, cache=FALSE, echo = FALSE, fig.cap='Log likelihood of the sample', out.width='80%', fig.asp=.75, fig.align='center'}
library(ggplot2)
cbp1 <- c("#999999", "#E69F00", "#56B4E9", "#009E73",
          "#F0E442", "#0072B2", "#D55E00", "#CC79A7")

ggplot(data.frame(x = c(0.01, 0.2)), aes(x = x)) +
  stat_function(fun = logLn, size = 1) +
  geom_point(aes(x=1/mean(t), y=logLn(1/mean(t))), colour=cbp1[2], size = 3) +
  theme_minimal() +
  xlab("lambda") + 
  ylab("log likelihood")
```
...
```{r hypTestIllustration, cache=FALSE, echo = FALSE, fig.cap='Log likelihood of the sample', out.width='80%', fig.asp=.75, fig.align='center'}
...
ggplot(data.frame(x = c(0.5, 1.5)), aes(x = x)) +
 ...
 geom_point(aes(x=0.9, y=f(0.9)), colour=cbp1[2], size = 3) +
...
```{r LR-testExp, cache=FALSE, echo = FALSE, fig.cap='Illustration of the likelihood ratio', out.width='80%', fig.asp=.75, fig.align='center'}
f <- function(l){ 
  l0 = 0.1
  200*(log(l/l0) + (l0-l)/l) 
}
...
```{r taylor-series, cache=FALSE, message=FALSE, fig.cap='Taylor series approximation of ln x', out.width='80%', fig.asp=.75, fig.align='center', echo = FALSE}
f <- log
...
```{r CI-plot, cache=FALSE, echo = FALSE, fig.cap='Score statistic and confidence intervall', out.width='80%', fig.asp=.75, fig.align='center'}
...
ggplot(data.frame(x = c(0.05, 0.2)), aes(x = x)) +
  ...
  geom_rect(aes(xmin=leftCILimit, xmax=rightCILimit, ymin=0, ymax=Inf), fill = cbp1[2], alpha = 0.2) +
  theme_minimal() +
...
```{r binRegLoadData, cache=FALSE, echo = FALSE}
...
```{r binRegrDataPlot, echo = FALSE, tidy=TRUE, cache=FALSE, message=FALSE, fig.cap = "Observed sample", out.width='80%', fig.asp=.75, fig.align='center'}
library(ggplot2)
...
```{r BinReglog-likelihood, cache=FALSE, echo = FALSE, fig.cap='Log likelihood of the sample', out.width='80%', fig.asp=.75, fig.align='center'}
library(ggplot2)
cbp1 <- c("#999999", "#E69F00", "#56B4E9", "#009E73",
          "#F0E442", "#0072B2", "#D55E00", "#CC79A7")

ggplot(data.frame(x = c(0, 4)), aes(x = x)) +
  stat_function(fun = logLOurSample, size = 1) +
  geom_point(aes(x=betahat, y=logLOurSample(betahat)), colour = cbp1[2], size = 3) +
  theme_minimal() +
  xlab("beta") + 
  ylab("log likelihood")
```

```{r binRegrDataPlotWithFit, echo = FALSE, tidy=TRUE, cache=FALSE, message=FALSE, fig.cap = "Observed sample and fitted model", out.width='80%', fig.asp=.75, fig.align='center'}
library(ggplot2)
f <- function(x){ s(betahat*x) }
ggplot(data.df, aes(x = x, y = y)) +
  geom_point() +
  stat_function(fun = f, colour = cbp1[2], size = 1) +
  theme_minimal()
```
...
```{r applicationScoreCI, echo = FALSE, tidy=TRUE, cache=FALSE, message=FALSE, fig.cap = "Observed sample and fitted model", out.width='80%', fig.asp=.75, fig.align='center'}
library(ggplot2)
library(latex2exp)
...
ggplot(data.frame(x = c(1, 3)), aes(x = x)) +
  stat_function(fun = Vectorize(zScore, "beta0"), size = 1) +
  geom_rect(aes(xmin=leftCILimit, xmax=rightCILimit, ymin=0, ymax=Inf), fill = cbp1[2], alpha = 0.2) +
  theme_minimal() +
  ylab('Score statistic') + 
  xlab(TeX("$\\beta$"))
